...
#ifdef HAKANSRC
	CpuTimer2Regs.PRD.all =  20000000;//30000;	// C tasks
#else
...
{
#ifdef HAKANSRC
    SciSendFloatTagged(&SpeedRef, &SciaRegs, TAGNUMBER1);
#endif
	//-----------------
...
interrupt void MotorControlISR(void)
{
	...
	EINT;
#ifdef HAKANSRC
	reference_error = pid_spd.term.Fbk-SpeedRef;
    //SciaUartSend_NoInterrupt("selamnaber", 10);
    //SciSendFloatNoTag(&pid_spd.term.Fbk, &SciaRegs);
	//SciSendFloatNoTag(&clarke1.As, &SciaRegs);
	SciSendFloatNoTag(&reference_error,&SciaRegs);
#endif
	// Verifying the ISR
    IsrTicker++;

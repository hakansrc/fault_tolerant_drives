...
var deferedSymbols = null; /* != null => ROV waiting for syms on active core */
var exeSet = null;         /* != null => ROV connected to the exe exeSet */
...
function setExecutable(path, reqObj, callback, progress, targetEvents)
{
    ...
    /* TI Cloud */
        ...
        var packagePath = []; /* package path array from CCS */
            ...
            ds.addListener("configChanged",
                function(data) {
                    ...
                    if (data.cores[0] == undefined) {
                        ...
                        if (exeSet != null || deferedSymbols != null) {
                            exeSet = null;
...
{
    var packagePathDefault = [];
    var xdcRootDefault;

    return (_reqPackagePath(null)
        .then(
            /* get default package path and xdcRootDefault */
            function (ppath) {
                xdcRootDefault = ppath[ppath.length - 1];
                packagePathDefault = ppath;
                return (ppath);
            }
        )
        .then(
            /* if necessary, get project-specific package path */
            function (ppath) {
        ...
        .then(
            ...
            function (ppath) {
                /* if proj isn't (or doesn't reference) a RTSC project */
                if (ppath == null || ppath.length == 0) {
                    console.log("the project '" + projectName
                                + "' doesn't specify a package path, using workspace"
                                + " package path ...");
                    return (packagePathDefault);
                }

                /* ensure xdctools is at the end of path, add it if needed */
                var last = ppath[ppath.length - 1];
                if (!last.match(/[\/\\]xdctools_.*packages\/*$/)) {
                    ppath.push(xdcRootDefault);
                }
                return (ppath);
            }
        )
        .fail(
            ...
            function (msg) {
                ...
                if (projectName != null
                    && msg != null && msg.indexOf("HTTP ERROR 400") != -1) {
                    console.log("the project '" + projectName

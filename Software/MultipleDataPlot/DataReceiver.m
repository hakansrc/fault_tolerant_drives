...
ProcessRawDataThresholdInBytes = 2048*50*10/4;    %the received data will be saved and/or converted and plotted when ProcessRawDataThresholdInBytes bytes of data is received
...
while(1)
    if(TheRawDataIsInProcessFlag==1)
        ...
        if(EnableSaving==1)
            FileName = sprintf("TestData/TestDataRL_Alignment_%s", datestr(now, 'ddmmyyHHMMSS'));
